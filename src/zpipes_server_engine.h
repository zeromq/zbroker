...
s_client_execute (s_client_t *self, int event)
{
    ...
    while (self->next_event != NULL_event) {
        ...
        if (self->server->animate) {
            zclock_log ("%6d: %s:", self->unique_id, s_state_name [self->state]);
            zclock_log ("%6d:     %s", self->unique_id, s_event_name [self->event]);
        }
        switch (self->state) {
            case start_state:
            ...
            case before_writing_state:
                ...
                if (self->event == error_event) {
                    ...
                    zlog_warning (self->server->log, "%6d: unhandled event %s in %s",
                        self->unique_id,
