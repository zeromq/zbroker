...
engine_set_log_prefix (client_t *client, const char *string)
{
    if (client) {
        s_client_t *self = (s_client_t *) client;
        snprintf (self->log_prefix, sizeof (self->log_prefix) - 1, "%6d:%-20s", self->unique_id, string);
    }
}
...
s_client_execute (s_client_t *self, int event)
{
    ...
    while (self->next_event != NULL_event) {
        ...
        if (self->server->animate) {
            zclock_log ("%s: %s:", self->log_prefix, s_state_name [self->state]);
            zclock_log ("%s:     %s", self->log_prefix, s_event_name [self->event]);
        }
        switch (self->state) {
            case start_state:
                if (self->event == output_event) {
                    if (!self->exception) {
                        ...
                        if (self->server->animate)
                            zclock_log ("%s:         $ lookup or create pipe", self->log_prefix);
                        lookup_or_create_pipe (&self->client);
                    }
                    if (!self->exception) {
                        //  open pipe writer
                        if (self->server->animate)
                            zclock_log ("%s:         $ open pipe writer", self->log_prefix);
                ...
                if (self->event == input_event) {
                    ...
                    if (!self->exception) {
                        //  open pipe reader
                        if (self->server->animate)
                            zclock_log ("%s:         $ open pipe reader", self->log_prefix);
                ...
                if (self->event == ping_event) {
                    if (!self->exception) {
                        //  send ping_ok
                        if (self->server->animate)
                            zclock_log ("%s:         $ send PING_OK", self->log_prefix);
                ...
                if (self->event == expired_event) {
                    if (!self->exception) {
                        //  terminate
                        if (self->server->animate)
                            zclock_log ("%s:         $ terminate", self->log_prefix);
                    ...
                    if (!self->exception) {
                        //  send invalid
                        if (self->server->animate)
                            zclock_log ("%s:         $ send INVALID", self->log_prefix);
            ...
            case before_writing_state:
                if (self->event == ok_event) {
                    if (!self->exception) {
                        //  send output_ok
                        if (self->server->animate)
                            zclock_log ("%s:         $ send OUTPUT_OK", self->log_prefix);
                ...
                if (self->event == error_event) {
                    if (!self->exception) {
                        //  send output_failed
                        if (self->server->animate)
                            zclock_log ("%s:         $ send OUTPUT_FAILED", self->log_prefix);
            ...
            case writing_state:
                if (self->event == write_event) {
                    if (!self->exception) {
                        ...
                        if (self->server->animate)
                            zclock_log ("%s:         $ process write request", self->log_prefix);
                ...
                if (self->event == close_event) {
                    if (!self->exception) {
                        //  close pipe writer
                        if (self->server->animate)
                            zclock_log ("%s:         $ close pipe writer", self->log_prefix);
                        close_pipe_writer (&self->client);
                    }
                    if (!self->exception) {
                        //  send close_ok
                        if (self->server->animate)
                            zclock_log ("%s:         $ send CLOSE_OK", self->log_prefix);
            ...
            case processing_write_state:
                if (self->event == have_reader_event) {
                    if (!self->exception) {
                        //  pass data to reader
                        if (self->server->animate)
                            zclock_log ("%s:         $ pass data to reader", self->log_prefix);
                        pass_data_to_reader (&self->client);
                    }
                    if (!self->exception) {
                        //  send write_ok
                        if (self->server->animate)
                            zclock_log ("%s:         $ send WRITE_OK", self->log_prefix);
                ...
                if (self->event == wakeup_event) {
                    if (!self->exception) {
                        //  send write_timeout
                        if (self->server->animate)
                            zclock_log ("%s:         $ send WRITE_TIMEOUT", self->log_prefix);
                ...
                if (self->event == close_event) {
                    if (!self->exception) {
                        //  send write_failed
                        if (self->server->animate)
                            zclock_log ("%s:         $ send WRITE_FAILED", self->log_prefix);
            ...
            case before_reading_state:
                if (self->event == ok_event) {
                    if (!self->exception) {
                        //  send input_ok
                        if (self->server->animate)
                            zclock_log ("%s:         $ send INPUT_OK", self->log_prefix);
                ...
                if (self->event == error_event) {
                    if (!self->exception) {
                        //  send input_failed
                        if (self->server->animate)
                            zclock_log ("%s:         $ send INPUT_FAILED", self->log_prefix);
            ...
            case reading_state:
                if (self->event == read_event) {
                    if (!self->exception) {
                        ...
                        if (self->server->animate)
                            zclock_log ("%s:         $ process read request", self->log_prefix);
                ...
                if (self->event == close_event) {
                    if (!self->exception) {
                        //  close pipe reader
                        if (self->server->animate)
                            zclock_log ("%s:         $ close pipe reader", self->log_prefix);
            ...
            case processing_read_state:
                if (self->event == have_data_event) {
                    if (!self->exception) {
                        ...
                        if (self->server->animate)
                            zclock_log ("%s:         $ collect data to send", self->log_prefix);
                        collect_data_to_send (&self->client);
                    }
                    if (!self->exception) {
                        //  send read_ok
                        if (self->server->animate)
                            zclock_log ("%s:         $ send READ_OK", self->log_prefix);
                ...
                if (self->event == zero_read_event) {
                    if (!self->exception) {
                        //  send read_end
                        if (self->server->animate)
                            zclock_log ("%s:         $ send READ_END", self->log_prefix);
                ...
                if (self->event == wakeup_event) {
                    if (!self->exception) {
                        //  send read_timeout
                        if (self->server->animate)
                            zclock_log ("%s:         $ send READ_TIMEOUT", self->log_prefix);
                ...
                if (self->event == close_event) {
                    if (!self->exception) {
                        //  send read_failed
                        if (self->server->animate)
                            zclock_log ("%s:         $ send READ_FAILED", self->log_prefix);
